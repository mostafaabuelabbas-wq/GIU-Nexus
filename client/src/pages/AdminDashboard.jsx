...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    api.get('/admin/stats')
      .then(({ data }) => {
        console.log('[admin/stats] raw response:', data)
        setRawResponse(data)
        setStats(data.stats || data)
      })
      .catch(err => {
        console.error('[admin/stats] error:', err.response?.status, err.response?.data)
        setError(`${err.response?.status || ''} ${err.response?.data?.message || err.message || 'Failed to load stats.'}`)
      })
  ...
  return (
    <div className="min-h-screen bg-[#F1EAD9] flex flex-col">
      ...
      <div className="max-w-[1360px] w-full mx-auto px-10 pt-10 pb-6">
        ...
        <p className="text-[#3B342B] text-[17px] mt-2">A live look at the platform.</p>
      </div>
      ...
      <div className="flex-1 max-w-[1360px] w-full mx-auto px-10 pb-16">
        ...
        {!loading && !error && stats && (
          <div className="flex flex-col gap-6">
            ...
            {/* ── Diagnostic banner (auto-shows when API returned but data is empty) ── */}
            {totalUsers + totalJobs + totalApps === 0 && (
              <div className="bg-[#FEF3C7] border border-[#E5A93A]/40 rounded-2xl p-5">
                ...
                <p className="text-[#7A3E1C] text-sm mb-3">
                  The <code className="bg-white/60 px-1.5 py-0.5 rounded">/admin/stats</code> endpoint replied, but with empty data. Raw response below — please share what you see here:
                </p>
                <pre className="bg-white/70 rounded-xl p-3 text-[12px] font-['JetBrains_Mono'] text-[#161310] overflow-auto max-h-64">
{JSON.stringify(rawResponse, null, 2)}
                </pre>
              </div>
            )}

            {/* ── Four large numeric tiles ── */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatTile
                ...
                value={totalUsers}
                tone="ink"
                hint={`${usersByRole.jobSeeker ?? 0} students · ${usersByRole.recruiter ?? 0} recruiters · ${usersByRole.admin ?? 0} admins`}
                href="/admin/users"
              />
              <StatTile
                label="Jobs"
                ...
                value={totalJobs}
                tone="palm"
                hint={`${jobsByStatus.open ?? 0} open · ${jobsByStatus.closed ?? 0} closed`}
                href="/admin/jobs"
              />
              <StatTile
                label="Applications"
                ...
                value={totalApps}
                tone="cream"
                hint={`${appsByStatus.pending ?? 0} awaiting review`}
              />
              <StatTile
                label="Shortlisted"
                arLabel="مختارون"
                value={appsByStatus.shortlisted ?? 0}
                tone="pink"
                hint={`${appsByStatus.rejected ?? 0} rejected`}
              />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <BreakdownCard
                title="Users by role"
                rows={[
                  { k: 'jobSeeker', l: 'Students',   color: '#EE5688', value: usersByRole.jobSeeker ?? 0, total: totalUsers },
                  { k: 'recruiter', l: 'Recruiters', color: '#E96A3A', value: usersByRole.recruiter ?? 0, total: totalUsers },
                  { k: 'admin',     l: 'Admins',     color: '#161310', value: usersByRole.admin     ?? 0, total: totalUsers },
                ]}
              />
              <BreakdownCard
            ...
            <div className="bg-white rounded-[28px] p-7 border border-[#16131010]">
              <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
                <div>
                  <p className="font-['JetBrains_Mono'] text-[11px] uppercase tracking-wider text-[#3B342B]/50">
                    Top jobs by applications
                  </p>
                  <h3 className="font-['Space_Grotesk'] font-bold text-[22px] text-[#161310] mt-0.5">
              ...
              {topJobs.length === 0 ? (
                <p className="text-[#3B342B]/60 text-sm py-6 text-center">
                  No applications yet — the leaderboard fills up once students start applying.
                </p>
              ) : (
                <ol className="flex flex-col gap-3">
                  {topJobs.map((job, i) => (
                    <li key={job._id ?? i} className="flex items-center gap-4">
                      <span className="font-['Space_Grotesk'] font-bold text-[28px] text-[#161310]/15 w-8 text-right flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          ...
                          {job.category && (() => { const { bg, text, dot } = getCategoryColor(job.category); return (
                            <span className="inline-flex items-center gap-1.5 rounded-full font-['JetBrains_Mono'] font-medium px-2 py-0.5 text-[10px]" style={{ background: bg, color: text }}>
                              <span className="w-1 h-1 rounded-full" style={{ background: dot }} />{job.category}
                            </span>
                          )})()}
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <div className="font-['Space_Grotesk'] font-bold text-[22px] text-[#EE5688] leading-none">
                          {job.applicationCount ?? job.count ?? 0}
                        </div>
                        <div className="font-['JetBrains_Mono'] text-[10px] uppercase tracking-wider text-[#3B342B]/40 mt-0.5">
                          apps
                        </div>
                      </div>
                    </li>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                { label: 'Browse platform',    href: '/jobs',             ar: 'تصفّح',    accent: '#161310' },
              ].map(q => (
                <Link key={q.label} to={q.href}
                  className="bg-white border border-[#16131010] rounded-[20px] p-5 flex flex-col gap-3 hover:-translate-y-0.5 transition-transform no-underline">
...
function StatTile({ label, arLabel, value, tone, hint, href }) {
  const palette = {
    ink:   { bg: '#161310', fg: '#F1EAD9', accent: '#E5A93A', sub: 'rgba(241,234,217,0.45)' },
    palm:  { bg: '#2F4A2E', fg: '#F1EAD9', accent: '#E5A93A', sub: 'rgba(241,234,217,0.45)' },
    pink:  { bg: '#EE5688', fg: '#FFFFFF', accent: '#FFFFFF', sub: 'rgba(255,255,255,0.65)' },
    cream: { bg: '#FFFFFF', fg: '#161310', accent: '#EE5688', sub: '#3B342B' },
  }[tone] || { bg: '#FFFFFF', fg: '#161310', accent: '#EE5688', sub: '#3B342B' }

  const inner = (
    <div
      className="rounded-[24px] p-6 border border-[#16131010] flex flex-col justify-between min-h-[170px] h-full hover:-translate-y-0.5 transition-transform"
      style={{ background: palette.bg }}
    >
      <div className="flex items-center justify-between">
        <p className="font-['JetBrains_Mono'] text-[11px] uppercase tracking-wider" style={{ color: palette.sub }}>
          {label}
        </p>
        {href && (
          <span className="font-['JetBrains_Mono'] text-[11px]" style={{ color: palette.accent }}>
            →
          </span>
        )}
      </div>
      <div>
        <div className="font-['Space_Grotesk'] font-bold text-[56px] leading-none" style={{ color: palette.fg }}>
          {value}
        </div>
        <p className="font-['Cairo'] font-bold text-base mt-1" style={{ color: palette.sub }} dir="rtl">
          {arLabel}
        </p>
      </div>
      <p className="font-['JetBrains_Mono'] text-[10px] uppercase tracking-wider mt-3" style={{ color: palette.sub }}>
        {hint}
      </p>
    </div>
  )
...
function BreakdownCard({ title, rows }) {
  return (
    <div className="bg-white rounded-[24px] p-7 border border-[#16131010]">
