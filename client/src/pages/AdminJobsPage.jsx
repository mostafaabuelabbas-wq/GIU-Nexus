...
const TYPE_L  = { 'full-time':'Full-time','part-time':'Part-time','internship':'Internship' }

const inputCls = 'bg-[#F1EAD9] border border-[#16131012] rounded-xl px-4 py-3 text-[#161310] text-[15px] placeholder-[#3B342B]/35 focus:outline-none focus:border-[#EE5688] focus:ring-2 focus:ring-[#EE5688]/15 focus:bg-[#F7F1E3] transition-all'
    ...
    <div className="min-h-screen bg-[#F1EAD9] flex flex-col">
      ...
      <div className="bg-[#F7F1E3] border-b border-[#16131010] relative overflow-hidden">
        ...
        <div className="max-w-[1360px] mx-auto px-10 py-10 relative z-10">
          <div className="flex items-baseline gap-3 flex-wrap">
            ...
            <span className="font-['Cairo'] font-black text-[#3B342B]/20" style={{ fontSize: 'clamp(28px,4vw,44px)' }}>الوظائف</span>
          </div>
          <p className="text-[#3B342B] text-[16px] mt-2">Search and delete any listing on the platform.</p>
        </div>
      </div>
      ...
      <div className="max-w-[1360px] w-full mx-auto px-10 pt-8 mb-6">
        <form onSubmit={handleSearch} className="flex gap-3">
          <input value={draftKw} onChange={e => setDraftKw(e.target.value)}
            placeholder="Search by title, company, or location…"
            className={`flex-1 ${inputCls}`} />
          <button type="submit"
            className="bg-[#2F4A2E] text-[#F1EAD9] font-bold text-sm px-6 py-3 rounded-xl hover:bg-[#243b23] transition-colors">
            Search →
          </button>
          {keyword && (
      ...
      <div className="flex-1 max-w-[1360px] w-full mx-auto px-10 pb-16">
        ...
        {!loading && !error && jobs.length === 0 && (
          <div className="text-center py-20 max-w-sm mx-auto">
            <span className="font-['JetBrains_Mono'] text-xs uppercase tracking-wider text-[#3B342B]/40 block mb-3">// no results</span>
            <h3 className="font-['Space_Grotesk'] font-bold text-2xl text-[#161310] mb-3">No jobs found.</h3>
            {keyword && <button onClick={clearSearch} className="text-[#EE5688] font-semibold hover:underline">Clear search →</button>}
          </div>
        )}
        ...
        {!loading && jobs.length > 0 && (
          <>
            ...
            <div className="flex flex-col gap-3">
              {jobs.map(job => {
                const { bg, text, dot } = job.category ? getCategoryColor(job.category) : {}
                const isOpen = job.status === 'open'
                return (
                  <article key={job._id}
                    className="bg-[#EDE4D0] rounded-[20px] px-5 py-4 border border-[#2F4A2E]/15 flex items-center gap-4 hover:border-[#2F4A2E]/30 transition-colors">
                    {/* status dot */}
                    <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${isOpen ? 'bg-[#16A34A] shadow-[0_0_0_3px_rgba(22,163,74,0.2)]' : 'bg-[#3B342B]/25'}`} />

                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <h3 className="font-['Space_Grotesk'] font-bold text-[15px] text-[#161310] truncate">{job.title}</h3>
                        {job.category && (
                        ...
                        <span className={`font-['JetBrains_Mono'] text-[10px] px-2 py-0.5 rounded-full uppercase tracking-wide ${isOpen ? 'bg-[#DCFCE7] text-[#15803D]' : 'bg-[#E4D9C4] text-[#3B342B]/60'}`}>
                          {job.status}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-[#3B342B] text-sm font-medium">{job.company}</span>
                        <span className="text-[#3B342B]/30 text-xs">·</span>
                        <span className="text-[#3B342B] text-xs">{job.location}</span>
                        {job.type && <><span className="text-[#3B342B]/30 text-xs">·</span><span className="text-[#3B342B] text-xs">{TYPE_L[job.type] ?? job.type}</span></>}
                      </div>
                      {(job.recruiter?.name || job.recruiter?.email) && (
                        <p className="font-['JetBrains_Mono'] text-[10px] uppercase tracking-wider text-[#3B342B]/35 mt-1">
                          Posted by: {job.recruiter?.name ?? job.recruiter?.email}
                        </p>
                      )}
                    </div>

                    <button onClick={() => { setDeleteError(''); setDeleteId(job._id) }}
      ...
      <Modal open={!!deleteId} title="Delete this job?"
